...
// Crée un event
const form = document.getElementById('form_new_event');
    ...
    if (!submitBtn.classList.contains('modifier')) {
        ...
        axios({
            ...
            .then(function (response) {
                ...
                document.querySelector("#form_new_event").reset();
            })
...
collectionRef.orderBy("date", "desc").onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        row.insertCell(2).innerHTML = doc.data().resume.substring(0, 20) + (doc.data().resume.length > 20 ? "..." : "");
...
function showUpdateModal(doc) {
    ...
    const submitBtn = document.getElementById("submit");
    submitBtn.textContent = "Modifier l'evenement";
